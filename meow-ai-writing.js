(() => {
  ...
  async function getAuth() {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error("Không tìm thấy phiên đăng nhập. Hãy đăng nhập lại."));
      }, 9000);
  ...
  async function gradeWriting(button) {
    ...
    if (!question) {
      ...
      resultBox.textContent = "Không đọc được đề bài của ô này.";
      return;
    }

    if (studentText.length < MIN_LENGTH) {
      resultBox.className = "ai-result error";
      resultBox.textContent = "Bài viết còn quá ngắn. Hãy viết thêm rồi gửi AI chấm.";
      return;
    }

    if (studentText.length > MAX_LENGTH) {
      resultBox.className = "ai-result error";
      resultBox.textContent = `Bài viết quá dài. Giới hạn hiện tại là ${MAX_LENGTH} ký tự.`;
      return;
    }
    ...
    resultBox.innerHTML = `<span class="ai-spinner"></span>AI đang đọc đề, kiểm tra yêu cầu và chấm bài...`;
    ...
    try {
      ...
      resultBox.className = "ai-result error";
      resultBox.innerHTML = `
        <b>Chưa chấm được bài.</b>
        <div style="margin-top:5px">${escapeHtml(err?.message || err)}</div>
        <div class="ai-meta">Kiểm tra Edge Function <b>grade-writing</b>, đăng nhập và GEMINI_API_KEY trong Supabase Secrets.</div>
      `;
    } finally {
